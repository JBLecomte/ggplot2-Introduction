...
<<setup, include=FALSE>>=
library(knitr)
opts_chunk$set(fig.path='figure/', fig.align='center', fig.show='hold', size='footnotesize', out.width='.9\\linewidth')
opts_template$set(
...
\begin{itemize}
% 			\item Points, as for a scatterplot \texttt{geom_point()}
        \item %Lines \texttt{geom_line()}
% 			\item Histogram \texttt{geom_bar()}
% 			\item Boxplot \texttt{geom_boxplot()}
% 			\item Polygon \texttt{geom_polygon}
% 			\item Draw rectangles \texttt{geom_raster}
% 			\item Smooth density estimate \texttt{geom_density}
% 			\item Map \texttt{ggmap()}
		\end{itemize}
\end{frame}
